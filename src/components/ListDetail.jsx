...
const SUGGESTIONS_DICTIONARY = [
    'Milk', 'Eggs', 'Butter', 'Garlic Butter', 'Peanut Butter', 'Amul Butter',
    'Cheese', 'Tofu', 'Onions', 'Yogurt', 'Bread', 'Coffee', 'Olive Oil',
    'Tomatoes', 'Lettuce', 'Beef patties', 'Buns', 'Charcoal'
];
...
export default function ListDetail({
    ...
    useEffect(() => {
        if (inputValue.trim() === '') {
            ...
            const filtered = SUGGESTIONS_DICTIONARY.filter(item =>
        ...
        <div className="list-detail-container">
            ...
            <div className="detail-header">
                ...
                <div className="header-right-actions">
                    ...
                    <div className="collaborators-badge" onClick={onOpenShare}>
                        ...
                        <button className="add-collab-btn" onClick={(e) => { e.stopPropagation(); onOpenShare(); }}>
            ...
            <div className="list-meta-info">
                {isEditingTitle ? (
                    ...
                    <h2 className="detail-list-title" onClick={handleTitleDoubleClick} title="Tıklayarak düzenleyin">
                        {list.title}
                    </h2>
                )}
                ...
                <div className="meta-row">
                    <div className="meta-item">
                        ...
                        <span>List {completedItems}/{totalItems} Completed</span>
                    </div>
                    ...
                    {isEditingTag ? (
                        ...
                        <div className="meta-item tag-badge" onClick={handleTagDoubleClick} title="Tıklayarak etiket ekleyin/düzenleyin">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                {totalItems === 0 && !isAddingInline ? (
                    <div className="empty-items-state">
                        <div className="empty-items-illustration">
                            <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="25" y="15" width="70" height="90" rx="8" fill="#F9FAFB" stroke="#EAECF0" strokeWidth="2" />
                                <rect x="40" y="8" width="40" height="14" rx="4" fill="#7F56D9" />
                                <line x1="40" y1="40" x2="80" y2="40" stroke="#EAECF0" strokeWidth="3" strokeLinecap="round" />
                                <line x1="40" y1="60" x2="80" y2="60" stroke="#EAECF0" strokeWidth="3" strokeLinecap="round" />
                                <line x1="40" y1="80" x2="70" y2="80" stroke="#EAECF0" strokeWidth="3" strokeLinecap="round" />
                                <circle cx="90" cy="90" r="16" fill="#F4EBFF" />
                                <path d="M86 90H94M90 86V94" stroke="#7F56D9" strokeWidth="2" strokeLinecap="round" />
                            </svg>
                        </div>
                    ...
                    <div className="checklist-stack">
                        {list.items.map((item) => (
                            <div
                                key={item.id}
                                className={`checklist-item-card ${item.checked ? 'checked' : ''}`}
                            >
                                <button
                                    className="checkbox-wrapper"
                                    onClick={() => list.canEdit && onToggleItem(list.id, item.id)}
                                    disabled={!list.canEdit}
                                    style={{ border: 'none', background: 'none', padding: 0, cursor: list.canEdit ? 'pointer' : 'default', display: 'flex', alignItems: 'center' }}
                                >
