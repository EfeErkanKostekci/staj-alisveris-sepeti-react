    ...
    <div className="notif-backdrop" onClick={onClose}>
      <div className="notif-panel" onClick={(e) => e.stopPropagation()}>
        <div className="notif-header">
          <h3>Bildirimler</h3>
          {invites.length > 0 && <span className="notif-count-badge">{invites.length}</span>}
        </div>
              ...
              <div key={invite.id} className="notif-card">
                ...
                <div className="notif-content">
                  <p className="notif-message">
                    <strong>{invite.inviterName}</strong> sizi <strong>"{invite.listTitle}"</strong>{' '}
                    listesine davet etti
                  </p>
                  <p className="notif-meta">
                    {invite.inviterEmail} · {invite.role === 'Edit' ? 'Düzenleme' : 'Görüntüleme'}{' '}
                    yetkisi
                  </p>
                  <div className="notif-actions">
                    <button className="notif-accept-btn" onClick={() => onAccept(invite.id)}>
                      Kabul Et
                    </button>
                    <button className="notif-decline-btn" onClick={() => onDecline(invite.id)}>
                      Reddet
                    </button>
                  </div>
                </div>
